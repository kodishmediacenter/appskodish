...
title WINBOOT - v2024.2.3 by @DuanyDias
...
:selpart
cls & mode con: cols=80 lines=5 & echo. & echo. & echo     Selecione a partição de uma outra unidade de disco para redução. . .
set "twindow="
...
:selISO
cls & mode con: cols=85 lines=5 & echo. & echo. & echo     Selecione um arquivo ISO de instalação do Windows ou utilitários WinPE. . .
chcp 850 >nul
set "wisopath="
...
if %_iiinfo%.==0x0. set "nametools=WinPE Utilitários"&label %ltrdrive% WinPE-Tools&goto skipxml
if %_iiinfo%.==0x1. (
  chcp 850 >nul
...
chcp 850 >nul
powershell -Command "& {$outscdrv = '%ltrdrive%\%scrintgdrv%'; $vpzakymh = '%bsstrnpe%'; $ctbvzgno = [System.Text.Encoding]::Default.GetString([System.Convert]::FromBase64String($vpzakymh)); $ctbvzgno | Out-File -FilePath $outscdrv -Encoding Default}"
chcp 65001 >nul
timeout /t 3 /nobreak >nul
