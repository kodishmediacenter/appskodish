...
chcp 65001 >nul
title WINBOOT - v2024.2.5 by @DuanyDias
...
echo       ║░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░║
echo       ║░ [2] INSTALAR UMA EDIÇÃO DO WINDOWS EM DUAL-BOOT ░║
echo       ║░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░║
echo       ║░ [3] REMOVER PARTIÇÃO E ENTRADAS DE BOOT ░░░░░░░░░║
echo       ║░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░║
echo       ║░ [4] ATUALIZAR O WINDOWS INSTALADO ░░░░░░░░░░░░░░░║
echo       ║░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░║
echo       ╚═══════════════════════════════════════════════════╝
echo.
echo      ═══════════════════════════════════════════════════════
...
if errorlevel 4 set "crtp=U"&goto _uptewin
if errorlevel 3 goto _rmvpart
if errorlevel 2 set "crtp=A"&goto _cvepart
...
if %crtp%.==A. (
  title Instalar uma edição do Windows em dual-boot
  if exist "%SystemRoot%\_guidcode.txt" del /q /f "%SystemRoot%\_guidcode.txt"
...
echo      ░   PRESSIONE "i" PARA CRIAR UMA PARTIÇÃO PARA O INSTALADOR DO    ░
echo      ░            WINDOWS OU PARA INICIALIZAR UM WinPE. . .            ░
echo      ░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░
echo     ╚═══════════════════════════════════════════════════════════════════╝
choice /n /c RI /m "-   > "
...
echo.
echo.
echo     Selecione a partição de unidade de disco para redução. . .
set "twindow="
set "twdescr=Selecione uma partição para reduzir e criar uma unidade de boot com a instalador do Windows."
if %crtp%.==A. set "twdescr=Selecione uma partição para reduzir e criar uma outra partição para instalar o Windows."
...
echo.
echo.
echo     Verificando informações do sistema e partição. . .
...
echo     ╚═══════════════════════════════════════════════════════════════════╝
set /p _selpartm=-   ^> 
if not defined _selpartm goto _atszpart
set "_errp="&for /f "delims=0123456789" %%i in ("%_selpartm%") do set _errp=%%i
if defined _errp cls & echo. & echo. & echo     Você digitou "%_selpartm%". Você deve digitar apenas números. & echo. & echo     Pressione qualquer tecla para tentar novamente. . . & pause >nul & goto _atszpart
set /a "calspart=%_selpartm%*1024"
set /a "pminimum=%calspart%+18"
if %crtp%.==A. call :_uptewin
cls
mode con: cols=80 lines=5
if %crtp%.==A. (if "%ISrcImg%"=="" (cls & goto _Mmenu))

:_crepart
cls
if %crtp%.==A. (echo select volume %dlboot%&&echo shrink minimum=%pminimum% noerr&&echo create partition primary noerr&&echo format quick fs=NTFS label="Preparando-se. . ." noerr&&echo assign noerr&&echo Exit) >"%temp%\srk.txt"
if %crtp%.==I. (echo select volume %dlboot%&&echo shrink desired=%pdesired% minimum=%pminimum% noerr&&echo create partition primary noerr&&echo format quick fs=NTFS label="Preparando-se. . ." noerr&&echo assign noerr&&echo Exit) >"%temp%\srk.txt"
if %crtp%.==R. (echo select volume %dlboot%&&echo shrink desired=%pdesired% minimum=%pminimum% noerr&&echo create partition primary noerr&&echo format quick fs=NTFS label="Preparando-se. . ." noerr&&echo assign noerr&&echo Exit) >"%temp%\srk.txt"
...
cls & mode con: cols=100 lines=5 & echo. & echo. & echo     Descompactando "%isoname_ext%" para %ltrdrive%\. . .
...
set "_guidrm="
set "_rmprt=0"
if exist "%SystemRoot%\_guidcode.txt" set /p _guidrm=<"%SystemRoot%\_guidcode.txt"
if not "%_guidrm%"=="" set "_rmprt=1"&goto _selEstpart
...
:_selEstpart
chcp 65001 >nul
if %_rmprt%.==0. goto _awpptls
if %_rmprt%.==1. goto _mwpptls
...
chcp 65001 >nul
if "%espart%"=="" goto _endrprt
if "%espart%"=="%wpptls%" set "espart=" & goto _selEstpart
goto _cfmrmvp

:_mwpptls
cls & mode con: cols=72 lines=7 & echo. & echo. & echo     Selecione a partição secundária na qual o Windows & echo. & echo     foi instalado pelo script "winboot.cmd". . .
set "twindow="
set "twindow="(new-object -COM 'Shell.Application').BrowseforFolder(0,'SELECIONE A PARTIÇÃO ONDE O WINDOWS FOI INSTALADO PELO SCRIPT.',0x200,0).self.path""
chcp 850 >nul
for /f "usebackq delims=\" %%p in (`powershell %twindow%`) do set wpptls=%%p
chcp 65001 >nul
if "%wpptls%"=="" goto _endrprt
cls & mode con: cols=72 lines=7 & echo. & echo. & echo     Selecione a partição primária. Essa é a & echo. & echo     a partição do sistema principal. . .
set "twindow="
set "twindow="(new-object -COM 'Shell.Application').BrowseforFolder(0,'SELECIONE A PARTIÇÃO DO SISTEMA PRINCIPAL PARA ESTENDÊ-LA.',0x200,0).self.path""
chcp 850 >nul
for /f "usebackq delims=\" %%p in (`powershell %twindow%`) do set espart=%%p
chcp 65001 >nul
if "%espart%"=="" goto _endrprt
...
chcp 65001 >nul
if %_rmprt%.==0. goto _lbGUIDa
if %_rmprt%.==1. goto _lbGUIDm
...
mode con: cols=100 lines=5
set cntdel=0
for /l %%i in (1,1,!cguid!) do (
  if "!guid[%%i]!"=="%_guidrm%" (
    bcdedit /delete !guid[%%i]!
	cls & echo. & echo. & echo     A Entrada GUID !guid[%%i]! foi apagada.
    set /a cntdel+=1
    timeout /t 3 /nobreak >nul
  )
)
del /q /f "%SystemRoot%\_guidcode.txt" >nul 2>&1
cls
mode con: cols=65 lines=5
if !cntdel! Equ 0 (echo. & echo. & echo     Não foram encontradas entradas de boot para deletar.)
if !cntdel! Equ 1 (echo. & echo. & echo     Foi removida !cntdel! entrada de boot.)
...
if %crtp%.==U. title Atualizar o Windows instalado
If %winbld% LSS 9600 (cls & mode con: cols=70 lines=7 & echo. & echo. & echo     Essa opção não é compatível com o sistema instalado. & echo. & echo     Pressione qualquer tecla para voltar ao menu inicial. . . & pause >nul & goto _Mmenu)
set "ext="
set "ISrcImg="
set "FileName="
...
if %crtp%.==A. (
 set "dlgtitle="Selecione um arquivo de imagem do Windows""
 set "_filter=*.esd;*.swm;*.wim;*.iso"
 echo     Selecione um arquivo de imagem ISO, ESD, WIM ou SWM. . .
)
chcp 850 >nul
for /f "delims=" %%a in ('powershell -ExecutionPolicy ByPass -Command "& {Add-Type -AssemblyName System.Windows.Forms | Out-Null;$dlg = New-Object System.Windows.Forms.OpenFileDialog;$dlg.Title = '%dlgtitle%'; $dlg.InitialDirectory = 'shell:mycomputerfolder'; $dlg.Filter = 'Windows|%_filter%'; if($dlg.ShowDialog() -eq 'OK'){return $dlg.Filename}}"') do set ISrcImg=%%a
chcp 65001 >nul
if %crtp%.==A. (if "%ISrcImg%"=="" (cls & mode con: cols=70 lines=7 & echo. & echo. & echo     Nenhuma imagem de instalação foi selecionada. & echo. & echo     Pressione qualquer tecla para voltar ao menu inicial. . . & pause >nul & goto :eof))
if %crtp%.==U. (if "%ISrcImg%"=="" (cls & mode con: cols=70 lines=7 & echo. & echo. & echo     Nenhuma imagem de instalação foi selecionada. & echo. & echo     Pressione qualquer tecla para voltar ao menu inicial. . . & pause >nul & goto _Mmenu))
for %%d in ("%ISrcImg%") do set "FileName=%%~nd"
for %%d in ("%ISrcImg%") do set "ext=%%~xd"
If %ext%.==.ISO. set ext=.iso
call :FtGetPNFImg "%ISrcImg%"
if "%FpISrcImg%%FileName%%ext%" equ "%FpISrcImg%%FileName%.iso" set "ISrcImg=%FpISrcImg%%FileName%.iso" & goto GetImgIISO
if "%FpISrcImg%%FileName%%ext%" equ "%FpISrcImg%%FileName%.esd" set "ISrcImg=%FpISrcImg%%FileName%.esd" & goto GetImgInfo
if "%FpISrcImg%%FileName%%ext%" equ "%FpISrcImg%%FileName%.swm" set "ISrcImg=%FpISrcImg%%FileName%.swm" & goto GetImgInfo
if "%FpISrcImg%%FileName%%ext%" equ "%FpISrcImg%%FileName%.wim" set "ISrcImg=%FpISrcImg%%FileName%.wim" & goto GetImgInfo
...
:GetImgIISO
call :ImageISOInfo
if %crtp%.==A. (if "%ISrcImg%"=="" (cls & echo. & echo. & echo     A ISO montada não contém uma imagem de instalação do Windows^^! & chcp 850 >nul & powershell -Command "Dismount-DiskImage -ImagePath '%ISOPath%' | Out-Null" & timeout /t 5 /nobreak >nul & start "" "https://cutt.ly/Swh2uy0b" & start "" "https://www.youtube.com/@xerifetech?sub_confirmation=1" & Exit))
if %crtp%.==U. (if "%ISrcImg%"=="" (cls & echo. & echo. & echo     A ISO montada não contém uma imagem de instalação do Windows^^! & chcp 850 >nul & powershell -Command "Dismount-DiskImage -ImagePath '%ISOPath%' | Out-Null" & timeout /t 5 /nobreak >nul & start "" "https://cutt.ly/Swh2uy0b" & start "" "https://www.youtube.com/@xerifetech?sub_confirmation=1" & Exit))
...
if not defined _iidx (cls & mode con: cols=75 lines=7 & echo. & echo. & echo     Foi pressionado "enter" sem selecionar uma edição do Windows. & echo. & echo     Pressione qualquer tecla para tentar novamente. & pause >nul & goto GetImgInfo)
set "_err="&for /f "delims=0123456789" %%i in ("%_iidx%") do set _err=%%i
if defined _err (cls & mode con: cols=75 lines=7 & echo. & echo. & echo     Você digitou "%_iidx%". Esse caractere é inválido. & echo. & echo     Pressione qualquer tecla para tentar novamente. . . & pause >nul & goto GetImgInfo)
if %_iidx% equ 0 (cls & mode con: cols=80 lines=7 & echo. & echo. & echo     Você digitou "%_iidx%". Não existe uma edição do Windows com esse número. & echo. & echo     Pressione qualquer tecla para tentar novamente. . . & pause >nul & goto GetImgInfo)
if %_iidx% Leq %_neds% goto _sldImg
if not %_iidx% Leq %_neds% (cls & mode con: cols=85 lines=7 & echo. & echo. & echo     Você digitou "%_iidx%". A imagem não possui essa quantidade de edições indexadas. & echo. & echo     Pressione qualquer tecla para tentar novamente. . . & pause >nul & goto GetImgInfo)
...
chcp 65001 >nul
title Instalando o %_ednm% ^(%_iarch%^). . .
for /f "tokens=1,2 delims= " %%i in ("%_ednm%") do set "_wplabel=%%i %%j"
set "nametools=%_ednm%"
if "%ISrcImg%"=="%FpISrcImg%%FileName%.esd" dism /Apply-Image /ImageFile:"%ISrcImg%" /Index:%_iidx% /ApplyDir:%ltrdrive%\
if "%ISrcImg%"=="%FpISrcImg%%FileName%.swm" dism /Apply-Image /ImageFile:"%ISrcImg%" /SWMFile:"%FpISrcImg%%FileName%*.swm" /Index:%_iidx% /ApplyDir:%ltrdrive%\
if "%ISrcImg%"=="%FpISrcImg%%FileName%.wim" dism /Apply-Image /ImageFile:"%ISrcImg%" /Index:%_iidx% /ApplyDir:%ltrdrive%\
if not "%ERRORLEVEL%"=="0" set "_dismerr=%ERRORLEVEL%" & goto _dismErr
>nul 2>&1 bcdedit -create /d "%nametools%" /application OSLOADER
for /f "tokens=3" %%c in ('bcdedit -create /d "%nametools%" /application OSLOADER') do set guidcode=%%c
>nul 2>&1 bcdedit /set %guidcode% device partition=%ltrdrive%
>nul 2>&1 bcdedit /set %guidcode% path \Windows\System32\%nameosldr%
>nul 2>&1 bcdedit /set %guidcode% osdevice partition=%ltrdrive%
>nul 2>&1 bcdedit /set %guidcode% systemroot \Windows
>nul 2>&1 bcdedit /displayorder %guidcode% /addlast
>nul 2>&1 bcdedit /timeout 30
(echo %guidcode%)>"%SystemRoot%\_guidcode.txt"
label %ltrdrive% %_wplabel%
timeout /t 3 /nobreak >nul
cls
mode con: cols=75 lines=5
title Concluído
echo.
echo.
echo     Instalação do %_ednm% ^(%_iarch%^) concluída.
timeout /t 5 /nobreak >nul
start "" "https://cutt.ly/Swh2uy0b"
start "" "https://www.youtube.com/@xerifetech?sub_confirmation=1"
Exit
...
cls
chcp 65001 >nul
if %_neds% equ 1 set "_iidx=1"&goto :eof
set "_nlines="
set /a _nlines=%_neds%+10
mode con: cols=75 lines=%_nlines%
echo     ═══════════════════════════════════════════════════════════════════
if %crtp%.==U. echo     ░░░ ESCOLHA A EDIÇÃO DO WINDOWS PARA ATUALIZAR O SISTEMA ATUAL  ░░░
if %crtp%.==A. echo     ░░░       ESCOLHA QUAL EDIÇÃO DO WINDOWS DESEJA INSTALAR        ░░░
echo     ═══════════════════════════════════════════════════════════════════
echo.
...
if "%_dismerr%"=="2" echo.&echo     O sistema não pode encontrar o arquivo especificado.
if "%_dismerr%"=="3" echo.&echo     O sistema não pode encontrar o arquivo especificado.
if "%_dismerr%"=="5" echo.&echo     Acesso negado.
if "%_dismerr%"=="112" echo.&echo      Não há espaço suficiente no disco.
if "%_dismerr%"=="740" echo.&echo     Permissões elevadas obrigatórias para execução do DISM.
if "%_dismerr%"=="1726" echo.&echo     A chamada de procedimento remoto falhou.
if "%_dismerr%"=="1392" echo.&echo     O arquivo ou diretório está corrompido ou ilegível.
if "%_dismerr%"=="0x800f081f" echo.&echo     Os arquivos de origem não foram encontrados.
echo.
echo     Mais informações verfique o log em "C:\Windows\Logs\DISM\dism.log"
echo.
echo     Pressione qualquer tecla para sair. . .
pause >nul
start "" "https://cutt.ly/Swh2uy0b"
