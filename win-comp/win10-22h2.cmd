...
set "BuildInfo=19045.4355"
set "lang_def=pt-br"
set "destDir=uup/!BuildInfo!/!lang_def!/all/amd64"
set "updateId=446f2934-e38b-4801-a529-fcf40231a02a"
if not exist "%rand%" mkdir %rand%
...
for /f "tokens=1 skip=2" %%a in ('find /i "Windows10.0-KB" "%aria2Script%"') do (
	set /a update_num=1
)
for /f "tokens=1 skip=2" %%a in ('find /i "Windows11.0-KB" "%aria2Script%"') do (
	set /a update_num=1
)
for /f "tokens=1" %%a in ('find /i "Edge" "%aria2Script%" ^| find ".wim"') do (
	set /a edge_num=1
	set edge_en=ON
)
set ser=
for /d %%A in (ServerAzureStackHCICor,ServerDatacenter,ServerStandard,ServerTurbine) do (
	for /f "tokens=1" %%g in ('find /i "out=Server" "%aria2Script%" ^| find /i "%%A"') do (
		set multu_num=0
		set multi_en=OFF
	)
)
for /f "tokens=1 skip=2" %%a in ('find /i ".appx" "%aria2Script%"') do (
	set /a apps_num=1
	set apps_en=ON
)
...
if %edge_num% neq 0 (echo  G. %lang_multi_g% - %edge_en%)
...
if %edge_num% neq 0 (if /i %menu%==G (if /i %edge_en%==OFF (set edge_en=ON) else (set edge_en=OFF)))
...
color 0a
call bin\convert-UUP.cmd %cd%\uup\!ser!!BuildInfo!\!lang_def!\all\amd64 %esd_en%
goto EOF
